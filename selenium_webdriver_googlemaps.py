...
# Firefox login is causing too many login errors, switching to Chrome going forward, but keeping FireFox as a back-up.
...
def parse_time_text_pair_and_get_max(time_text):
    ...
    For the cases: '15 - 20 min' , '1 - 2 h' the algorithm always returns the last part.
...
def create_webdriver(proxy_url=None):
    ...
    if proxy_url is not None:
		...
		#Firefox login is causing too many login errors, switching to Chrome
...
def create_firefox_profile(firefox_profile_path=None):
    ...
    @param firefox_profile_path: Path to Firefox profile. 
